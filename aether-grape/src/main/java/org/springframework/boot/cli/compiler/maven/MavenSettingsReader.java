...
public class MavenSettingsReader {
    ...
    private File getSettingsFile() {
        String settingsFile = System.getProperty("mavenSettings");
        return settingsFile != null && !settingsFile.isEmpty() ?
                new File(settingsFile) : new File(this.homeDir, ".m2/settings.xml");
    }

    private File getSettingsSecurityFile() {
        String settingsSecurityFile = System.getProperty("mavenSettingsSecurity");
        final String SECURITY_XML = ".m2/settings-security.xml";
        return settingsSecurityFile != null && !settingsSecurityFile.isEmpty() ?
                new File(settingsSecurityFile) :
                new File(homeDir, SECURITY_XML);
    }

    private class SpringBootSecDispatcher extends DefaultSecDispatcher {

        SpringBootSecDispatcher() {
            File file = getSettingsSecurityFile();
